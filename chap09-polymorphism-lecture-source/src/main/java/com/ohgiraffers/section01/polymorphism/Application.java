...
public class Application {
    ...
    public static void main(String[] args){
        ...
        // 다형성
        // 다형성이란 하나의 인스턴스가 여러 가지 타입을 가질 수 있는 것을 의미힌다.
        ...
        Animal animal1 = new Rabbit(); // Animal에(그릇) Rabbit() 생성 : jump() 메소드 사용할수 없는 상태
        Animal animal2 = new Tiger();

//        Rabbit r1 = new Animal();
//        Rabbit r1 = (Rabbit) new Animal(); // Animal을 Rabbit형으로 형변환 해주어야한다.

        System.out.println("동적 바인딩"); // 동적 바인딩 : (런타임 단계)
        System.out.println(animal1.getClass());
        animal1.cry();
        animal2.cry();

//        animal.jump(); // 현재 jump() 메소드를 사용할 수 없는 상태이므로 오류
        ((Rabbit)animal1).jump(); // Rabbit형으로 형변환 해주어서 jump 메서드 사용 가능해짐 (동적 바인딩이 아님. 컴파일 단계)
        ((Tiger)animal2).bite();
...
//        ((Rabbit)animal2).jump();
        ...
        System.out.println("animal 이 Tiger와 같은지 확인 : " + (animal1 instanceof Tiger));
        System.out.println("animal 이 Rabbit와 같은지 확인 : " + (animal1 instanceof Rabbit));
        System.out.println("animal1 이 Animal과 같은지 확인 : " + (animal1 instanceof Animal));
        System.out.println("animal 이 Rabbit과 같은가 : " + (animal instanceof Rabbit));
        System.out.println("animal 이 Tiger와 같은가 : " + (animal instanceof Tiger));
