...
public class Application01 {
    ...
    public static void main(String[] args) {
        ...
        // 행동하도록 한다는 것이 현실 세계와 다른 것이다. (캡슐화)
        // 모든 객체는 책임을 가지고 자신의 역할을 수행해야 한다.
        // 또한 하나의 객체는 너무 많은 일을 수행하는 것이 아닌 적절한 책임을 가지고 있어야 한다. (단일 책임의 원칙)

        // 객체와 객체의 상호작용이란?
        // 객체와 객체는 메세지를 통해 서로 상호작용을 한다. 메세지(메소드 호출)를 보내는 쪽을 송신이라고 하고
        // 메세지를 수신하는 쪽을 수신자라고 한다. 수신자는 메세지를 전달 받아 그 메세지에 해당하는 내용을 처리하는 방법을
        // 스스로 결정하고, 그 결정한 방법 대로 명령어 들을 순차적으로 기술한 것을 메소드라고 한다.
        // 상호작용에 중효한 것은 메세지를 송수신 하고 그 방법을 기술하여 메세지에 응답하는 상호작용을 설계하는 것이다.
        // 따라서 객체를 설계할 때는 속성은 부가적이며 적적한 책임을 가진 객체들간의 주고 받는 메세지를 이용해 상호작용하는 것을
        // 설계하는 것을 의미힌다.
        // 즉, 필드보다는 메소드를 중점적으로 추상화 기법을 적용하여 객체를 설게하고, 그 객체를 구현하는 방법으로 설계한 대로 클래스를
...
//        carRacer.startUp();
        ...
        loop : while (true) {
            ...
            switch (no) {
                ...
                case 2 : carRacer.stepAcceclator(); break;
                case 3 : carRacer.setUpBreak(); break;
                case 4 : carRacer.turnOff(); break;
                case 9 : System.out.println("프로그램을 종료합니다. "); break loop; // loop while 을 break 함
                default :
                    System.out.println("잘못된 번호를 선택하셨습니다. 다시 선택해주세요"); break;
            }
//            if ( no == 9){
//                break;
//            }
    ...
    //          필요한 인터페이스(메소드)만 노출하는 것을 의미힌다.

    // 모델링 : 추상화는 현실 세계의 개념을 소프트웨어 모델로 반환하며 이 모델은 소프트웨어 개발에서 사용된다.
    //         복잡한 현실 세계를 이해하고 해결책을 제공하는 도구로 사용된다.

    // 객체지향 프로그래밍 : 추상화는 객체지향 프로그래밍의 핵심 개념 중 하나로 클래스와 객체를 사용하여 추상화를 통해 현실 세계의 개념을 모델링하고
    //                    이를 소프트웨어로 구현한다. 클래스는 추상 데이터 타입을 정의하고, 객체는 클래사의 인스턴스로 실제 데이터와 메서드를 갖는다.
